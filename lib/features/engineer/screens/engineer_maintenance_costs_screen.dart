...
class EngineerMaintenanceCostsScreen extends StatefulWidget {
  const EngineerMaintenanceCostsScreen({super.key});
  @override
  State<EngineerMaintenanceCostsScreen> createState() =>
      _EngineerMaintenanceCostsScreenState();
}

class _EngineerMaintenanceCostsScreenState
    extends State<EngineerMaintenanceCostsScreen>
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      Navigator.pop(ctx);
                      try {
                        await ApiService.patch('/maintenance-costs/${cost['id']}', data: {
                          'costType': costType,
                          'amount': double.tryParse(amtCtrl.text),
                          'description': descCtrl.text,
                          if (invCtrl.text.isNotEmpty) 'invoiceNumber': invCtrl.text,
                        });
                        _load();
                      } catch (_) {}
                    },
  ...
  Future<void> _delete(String id) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('هل تريد حذف هذه التكلفة؟', style: AppText.body,
            textDirection: TextDirection.rtl),
        actions: [
    ...
    if (ok == true) {
      try { await ApiService.delete('/maintenance-costs/$id'); _load(); } catch (_) {}
    }
  }
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return GestureDetector(
                          ...
                          onLongPress: () => _delete(c['id'].toString()),
                          child: GlassCard(
